...
//! **Features:**
//!
//! * Call Rust functions from Wolfram code using [`#[export]`][crate::export].
...
//!   macro. Build `Expr` values concisely with the [`expr!`][crate::expr::expr] macro.
//! * Pass **typed Rust structs** directly using WXF serialization via
//!   `#[export(wxf)]` — no manual serialization code needed (see
//!   [`wolfram_export`](https://docs.rs/wolfram-export) with `features = ["wxf"]`).
...
//!
//! **WXF** functions (available via the `wolfram-export` crate with `features = ["wxf"]`)
//! pass a single WXF-encoded `ByteArray` in and out. Argument structs just need
//! `#[derive(FromWXF)]`; return types need `#[derive(ToWXF)]`. The kernel side calls
//! `BinaryDeserialize` / `BinarySerialize` automatically.
//!
//!
//!
//!
...
//! * [How To: Export typed functions using WXF][docs::using_wxf_mode]
//! * [`expr!`][crate::expr::expr] — build `Expr` values with WL-like syntax
//!
...
/// ```
/// # mod scope {
/// use wolfram_library_link::{self as wll, NumericArray, expr::{expr, Expr}};
///
...
/// ```
///
/// WXF-mode functions (from `wolfram-export` with `features = ["wxf"]`) are also
/// included automatically. They appear in the loader output as:
///
/// ```wolfram
/// "myFn" -> LibraryFunction[lib, "myFn", {LibraryDataType[ByteArray]}, LibraryDataType[ByteArray]]
/// ```
///
/// and are called from Wolfram using `BinarySerialize` / `BinaryDeserialize` automatically
/// when loaded via the paclet loader generated by `generate_loader!`.
///
