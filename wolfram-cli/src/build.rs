...
fn write_package_wl_files(
    ...
    // No dylib had any `#[export]` entries in its manifest — this is almost
    // always a misconfiguration (e.g. `wolfram-library-link` built with
    // `default-features = false` without re-adding
    // `automate-function-loading-boilerplate`, which is what makes `#[export]`
    // register into the manifest `cargo wl build` reads) rather than a library
    // that intentionally exports nothing. Leave a pointer in the generated
    // file so it's not silently mistaken for a working, empty package.
    let note = if libraries.is_empty() {
        Some(
            "(* No exported functions were discovered. If you expected functions here, \
             check that the `automate-function-loading-boilerplate` feature of \
             `wolfram-library-link` is enabled \u{2014} it is required for `#[export]` to \
             register functions so `cargo wl build` can find them. *)\n",
        )
    } else {
        None
    };
    write_wl_with_note(lib_dir.join("Functions.wl"), &functions, note)?;
...
fn write_wl(path: PathBuf, expr: &Expr) -> Result<()> {
    write_wl_with_note(path, expr, None)
}

/// Like [`write_wl`], but allows an extra WL comment line to be inserted
/// between the "do not edit" banner and the expression.
fn write_wl_with_note(path: PathBuf, expr: &Expr, note: Option<&str>) -> Result<()> {
