...
fn borrowed_struct_roundtrips_zero_copy() {
    let bytes = to_wxf(
        &Frame { name: "hello", payload: &[1u8, 2, 3, 0xff], count: 42 },
        None,
    )
    .unwrap();
    ...
    assert!(buf_range.start <= name_ptr && name_ptr < buf_range.end,
            "name should borrow the input buffer");
    assert!(buf_range.start <= payload_ptr && payload_ptr < buf_range.end,
